...
public class MapPanel extends JPanel {
	...
	public MapPanel(Map m, InfraRed ir, JFrame f) {
		...
		this.setPreferredSize(new Dimension(myMap.getSizeX()*LENGTHSCALE + (int)f.getBounds().getWidth(), myMap.getSizeY()*LENGTHSCALE+ (int)f.getBounds().getHeight()));
	...
	public void paint(Graphics g) {
		...
		//g.fillRect((int)(myMap.getPosX()/lengthX*LENGTHSCALE), this.getHeight() - 5 - (int)(myMap.getPosY()/lengthX*LENGTHSCALE), 5, 5);
		...
		for (int i = 0; i < 5; i++) {
			...
			if (sensorVal != Double.POSITIVE_INFINITY) {
				...
				dy = this.getHeight() - (myMap.getPosY() + sensorVal*Math.sin((angle-(i-2)*diffSensor)))/lengthX*LENGTHSCALE;
				g.drawLine((int)(myMap.getPosX()/lengthX*LENGTHSCALE),
						   (int)(this.getHeight() - myMap.getPosY()/lengthX*LENGTHSCALE),
						   (int)dx,
						   (int)dy);
			}
		}
